...
void display()
{
	...
	gluLookAt(0, 2.0,-3.0, -1, 0, 0, 0, 1.0, 0);
...
bool Material::loadTexture(const char* filename) {
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	std::cout << "Loaded texture: " << filename << ", size: " << width << "x" << height << ", channels: " << nrChannels << std::endl;
...
void subdivideModel(std::unique_ptr<model_t>& model) {
	if (model->mymesh.empty() || model->vertex.empty()) return;

	std::vector<Vertex> newVertices = model->vertex;
	std::vector<MMesh> newMeshes;

	// Edge midpoint map
	std::map<Edge, int> edgeMidpoints;

	for (const auto& mesh : model->mymesh) {
		std::vector<int> vertices;
		if (mesh.V1 >= 0) vertices.push_back(mesh.V1 - 1);
		if (mesh.V2 >= 0) vertices.push_back(mesh.V2 - 1);
		if (mesh.V3 >= 0) vertices.push_back(mesh.V3 - 1);
		if (mesh.V4 >= 0) vertices.push_back(mesh.V4 - 1);
		...
		for (int v : vertices) {
			faceCenter.X += model->vertex[v].X;
			faceCenter.Y += model->vertex[v].Y;
			faceCenter.Z += model->vertex[v].Z;
		}
		faceCenter.X /= vertices.size();
		faceCenter.Y /= vertices.size();
		faceCenter.Z /= vertices.size();

		int faceCenterIdx = newVertices.size();
		...
		for (size_t i = 0; i < vertices.size(); ++i) {
			...
			if (edgeMidpoints.find(edge) == edgeMidpoints.end()) {
				Vertex midpoint;
				midpoint.X = (model->vertex[v1].X + model->vertex[v2].X) / 2.0f;
				midpoint.Y = (model->vertex[v1].Y + model->vertex[v2].Y) / 2.0f;
				midpoint.Z = (model->vertex[v1].Z + model->vertex[v2].Z) / 2.0f;

				edgeMidpoints[edge] = newVertices.size();
				newVertices.push_back(midpoint);
			}
			edgePoints.push_back(edgeMidpoints[edge]);
		}

		// Create new faces
		for (size_t i = 0; i < vertices.size(); ++i) {
			MMesh newMesh;
			newMesh.V1 = vertices[i] + 1;  // OBJ indices start from 1
			newMesh.V2 = edgePoints[i] + 1;
			newMesh.V3 = faceCenterIdx + 1;
			newMesh.V4 = edgePoints[(i + vertices.size() - 1) % vertices.size()] + 1;

			// Copy material and texture info
			newMesh.m = mesh.m;
			newMesh.T1 = mesh.T1;
			newMesh.T2 = mesh.T2;
			newMesh.T3 = mesh.T3;
	...
	model->vertex = newVertices;
	model->mymesh = newMeshes;
}
class QuadricError {
public:
	float a, b, c, d, e, f, g, h, i, j;

	QuadricError() : a(0), b(0), c(0), d(0), e(0), f(0), g(0), h(0), i(0), j(0) {}

	QuadricError(float nx, float ny, float nz, float d_val) {
		a = nx * nx; b = nx * ny; c = nx * nz; d = nx * d_val;
		e = ny * ny; f = ny * nz; g = ny * d_val;
		h = nz * nz; i = nz * d_val;
		j = d_val * d_val;
	}
	...
	QuadricError operator+(const QuadricError& other) const {
		QuadricError result;
		result.a = a + other.a; result.b = b + other.b; result.c = c + other.c; result.d = d + other.d;
		result.e = e + other.e; result.f = f + other.f; result.g = g + other.g;
		result.h = h + other.h; result.i = i + other.i; result.j = j + other.j;
		return result;
	}
		...
		if (strncmp(buffer, mtllib, strlen(mtllib)) == 0) {
			...
			while (fgets(bufferm, sizeof(bufferm), fmp)) {
				...
				if (count == 4) {
					...
					if (count == 1) {
						std::cout << "Loading texture: " << buffers << std::endl;
						char mkdfilename[300];
						strcpy(mkdfilename, modeldir.c_str());
						strcat_s(mkdfilename, sizeof(mkdfilename), buffers);
						nm->material.at(mtx_idx)->loadTexture(mkdfilename);
				...
				count = sscanf(bufferm, "map_Ka %s\n", buffers);
				if (count == 1) {
					std::cout << "Loading ambient texture (map_Ka): " << buffers << std::endl;
					char mkafilename[300];
					strcpy(mkafilename, modeldir.c_str());
					strcat_s(mkafilename, sizeof(mkafilename), buffers);
					nm->material.at(mtx_idx)->loadTexture(mkafilename);
				}
...
void load_bench() {
	...
	bench_temp->translation(2, 0, 1.2);
	bench_temp->rotation_a(0, 1, 0, 105);
	models.push_back(move(bench_temp));
	bench_temp = make_unique<model_t>(bench);
	bench_temp->translation(2.3, 0, 0);
	bench_temp->rotation_a(0, 1, 0, 105);
	models.push_back(move(bench_temp));
}

void load_tree() {
	auto tree = load_model("tree01.obj", "models\\tree\\", 2);
	auto rand_place = make_random_place(1.0f, 3.0f, -3.0f, -1.5f, 3, 1, 0.5, 14);

	for (const auto& p : rand_place) {
		auto tree_instance = make_unique<model_t>(tree);
		tree_instance->translation(p.x, 0.05f, p.z);
		tree_instance->rotation_a(0, 1, 0, p.r);
		models.push_back(move(tree_instance));
	}
}

void load_dog() {
	auto dog = load_model("dog.obj", "models\\dog\\", 0.6f); // 스케일은 필요에 따라
	subdivideModel(dog);
	auto dog_instance = make_unique<model_t>(dog);
	dog_instance->translation(1.0f, 0.1f, 0.0f);
	dog_instance->rotation_a(1.0f, 0.0f, 0.0f, -90.0f); // x축으로 90도 회전
	models.push_back(move(dog_instance));
}

void load_swing() {
	auto swing = load_model("10549_ChildrenSwingSet_v1-LoD2.obj", "models\\swing\\", 1.6f); // 스케일은 필요에 따라
	subdivideModel(swing);
	auto swing_instance = make_unique<model_t>(swing);
	swing_instance->translation(-0.3f, 0.1f, 0.0f);
	swing_instance->rotation_a(1.0f, 0.0f, 0.0f, -90.0f); // x축으로 90도 회전
	models.push_back(move(swing_instance));
}

void load_cottage() {
	auto cottage = load_model("cottage_blender.obj", "models\\cottage\\", 5.0f); // 스케일은 필요에 따라
	subdivideModel(cottage);
	auto cottage_instance = make_unique<model_t>(cottage);
	cottage_instance->translation(5.5f, 0.1f, 0.0f);
	cottage_instance->rotation_a(0.0f, 1.0f, 0.0f, 20.0f); // x축으로 90도 회전
	models.push_back(move(cottage_instance));
}
